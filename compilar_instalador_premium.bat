...
if not exist "bazarnicole_Premium_Setup.iss" (
    echo ❌ ERROR: No se encuentra bazarnicole_Premium_Setup.iss
    pause
    exit /b 1
)
...
if not exist "installer_assets\app_icon.ico" (
    ...
    if exist "assets\image\logobazasr.png" (
        echo   → Necesitas convertir logobazasr.png a .ico manualmente
...
if not exist "build\windows\x64\runner\Release\bazarnicole.exe" (
    echo ❌ ERROR: No se encuentra bazarnicole.exe
...
"C:\Program Files (x86)\Inno Setup 6\ISCC.exe" "bazarnicole_Premium_Setup.iss"
...
echo.
echo 📁 Ubicación:
echo    installer_output\bazarnicole_Setup_v2.0.0.exe
echo.
echo 📊 Información:
if exist "installer_output\bazarnicole_Premium_Setup.exe" (
    for %%I in ("installer_output\bazarnicole_Premium_Setup.exe") do (
...
if errorlevel 2 (
    ...
    echo installer_output\bazarnicole_Premium_Setup.exe
    echo.
    pause
    exit /b 0
)
...
start "" "installer_output\bazarnicole_Premium_Setup.exe"
echo.
pause
exit /b 0