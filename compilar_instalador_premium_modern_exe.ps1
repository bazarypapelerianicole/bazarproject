...
$outputFile = Join-Path $repoRoot "installer_output\BazarPapeleria_Instalador_Premium_Modern.exe"
...
if (-not (Test-Path (Join-Path $buildSource "bazarnicole.exe"))) {
    throw "No se encontro build\\windows\\x64\\runner\\Release\\bazarnicole.exe. Ejecuta flutter build windows --release antes de compilar el EXE."
}
...
$optionalEmbeds = @(
    ...
    @{ Target = ".\\assets\\image\\AutoRepoLogo.png"; Candidates = @(
            (Join-Path $repoRoot "assets\image\AutoRepoLogo.png")
        ) },
    @{ Target = ".\\assets\\image\\Logo.jpeg"; Candidates = @(
            (Join-Path $repoRoot "assets\image\Logo.jpeg")
        ) }
)
...
$compilerParams = @{
    ...
    title = "Instalador Grupo El Gringo"
    description = "Instalador premium WinForms de Grupo El Gringo"
    company = "Grupo El Gringo"
    product = "Grupo El Gringo"
    version = "2.0.0"
}
